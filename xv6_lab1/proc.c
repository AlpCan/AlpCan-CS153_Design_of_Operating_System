...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        ...
        /*// Aging everything except the first item with max priority
        first_max = 1;
        for (p2 = ptable.proc; p2 < &ptable.proc[NPROC]; p2++) {
            if (p2->prior_val == max_prior && first_max) {
                first_max = 0;
                continue;
            }
            ...
            p2->prior_val = (p2->prior_val > 0) ? p2->prior_val - 1 : 0;
        }*/
